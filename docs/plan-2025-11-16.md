...
**変更履歴**: 2025-11-16に`src/blog/blog/`から`src/blog/`に構造を変更。これに伴い`src/lib/blog.ts`の`BLOG_DIR`定数を更新済み。
...
**Content Root Directory**: `process.cwd()/src/blog`から相対パスで記事を読み込む
...
### Jupyter Bookからの移行理由（廃止決定）
**決定日**: 2025-11-16
**ステータス**: ✅ 移行完了 - Jupyter Bookは使用していません
...
#### 現在のアプローチの利点
- **単一言語**: TypeScript/JavaScriptのみで完結
- **統合されたビルド**: Next.jsのビルドプロセスに完全統合
- **開発体験**: ホットリロード、TypeScript型チェック
- **デプロイの簡素化**: Dockerイメージも単一のNode.jsベース
- **パフォーマンス**: 静的サイト生成（SSG）により高速なページ読み込み
- **メンテナンス性**: シンプルな依存関係で長期的な保守が容易

#### 実装方法
Next.js App Router + Markdown処理ライブラリ（gray-matter, remark, rehype）を使用し、`src/blog/`内のMarkdownファイルを直接レンダリング。Jupyter Bookは完全に廃止。
